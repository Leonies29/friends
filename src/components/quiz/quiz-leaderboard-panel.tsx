...
export function QuizLeaderboardPanel({ entries }: { entries: QuizLeaderboardEntry[] }) {
  if (!entries.length) {
    return <Card><p className="text-sm text-muted-foreground">Personne n'a encore joué au quiz.</p></Card>;
  }

  return (
    <Card>
      <Badge>Classement Quiz</Badge>
          ...
          <div key={entry.id} className="flex items-center gap-3 rounded-2xl border border-border bg-background px-3 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs font-semibold text-muted-foreground">
                {entry.correctCount}/{entry.totalAnswered} bonnes · {entry.successRate}% réussite
              </p>
            </div>
